...
class Order(models.Model):
	...
	def detail(self):
		url = '<a href="/adminorders/%s">Change Status</a>' %(self.pk)
		return mark_safe(url)
	...
	def get_pincode(self):
		if self.billing_address is not None:
			return self.billing_address.postcode
		else:
			return None

	def get_commission(self):
		cart = self.cart.cartitem_set.all()
		total_commission = 0.00
		for ca in cart:
			total_commission += ca.quantity * ca.item.product.get_single_product_commission()
