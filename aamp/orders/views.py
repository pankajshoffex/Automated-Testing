...
from products.models import Availability, Product
...
import easy_pdf
import datetime
from sms.models import SendSMS, SmsHistory, AdminContact
...
def cancel_order(request,pk):
	...
	if request.method == "POST":
		...
		order.save()
		instance.save()
		msg = "your order %s has been cancelled due to %s" %(order.order_id, instance.reason)
		obj = SendSMS()
		result = obj.sendsms(msg, instance.user)
		if result:
			user = User.objects.get(username=instance.user)
			SmsHistory.objects.create(
				number=order.billing_address.mobile,
				recipient=order.billing_address.full_name,
				sms_subject=order.status,
				sms_text=msg,
...
def admin_orders(request, pk):
	...
	if form.is_valid():
		...
		if order.status == "received":
			...
			message = "Dear %s your order ID is %s of Rs. %s is received Happy Shopping!!! www.shoffex.com" %(order.billing_address.mobile, order.order_id, order.order_total)
			if admin_obj:
				for item in admin_obj:
					obj.sendsms(admin_msg, item.number)
			result = obj.sendsms(message, order.billing_address.mobile)
		elif order.status == "dispatched":
			order.status = "dispatched"
			admin_msg = "Status of %s's order %s has changed to dispatched." % (order.billing_address.full_name, order.order_id)
			message = "Dear %s your order %s is dispatched through shoffex carrier. Have a nice day!!! www.shoffex.com." %(order.billing_address.mobile, order.order_id)
			if admin_obj:
				for item in admin_obj:
					obj.sendsms(admin_msg, item.number)
			result = obj.sendsms(message, order.billing_address.mobile)	
		elif order.status == "intransist":	
			order.status = "intransist"
			admin_msg = "Status of %s's order %s has changed to intransist." % (order.billing_address.full_name, order.order_id)
			message = "Dear %s your order %s is intransist. The order will be delivered in given time. Have a nice day!!! www.shoffex.com." %(order.billing_address.mobile, order.order_id)
			if admin_obj:
				for item in admin_obj:
					obj.sendsms(admin_msg, item.number)
			result = obj.sendsms(message, order.billing_address.mobile)	
			...
			admin_msg = "Status of %s's order %s has changed to delivered" % (order.billing_address.full_name, order.order_id)
			message = "Dear %s Thank you for shopping with Shoffex.com Your order is delivered successfully!!! Cheers www.shoffex.com" %(order.billing_address.mobile)
			if admin_obj:
				for item in admin_obj:
					obj.sendsms(admin_msg, item.number)
			result = obj.sendsms(message, order.billing_address.mobile)
...
def admin_cancel_order(request, pk):
	...
	if request.method == "POST":
		...
		order.save()
		instance.save()
		message = "Dear %s Your order %s has been cancelled due to %s. We are sorry for inconvenience. www.shoffex.com" %(order.billing_address.mobile, order.order_id, instance.admin_reason)
		obj = SendSMS()
		result = obj.sendsms(message, order.billing_address.mobile)
		if result:
			SmsHistory.objects.create(
